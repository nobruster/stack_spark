"""
╔══════════════════════════════════════════════════════════════════╗
║         CAMADA OURO — Benefícios Emitidos PDA 2025-2027         ║
╚══════════════════════════════════════════════════════════════════╝
...
─── 4. PERFORMANCE ────────────────────────────────────────────────
  ...
  • shuffle.partitions=12: adequado para cores.max=2
...
print("=" * 65)
print("  OURO — Benefícios Emitidos PDA 2025-2027")
print("=" * 65)
...
spark = (
    ...
    .appName(f"Gold-PDA-Beneficios-{ANO_MES}")
    .config("spark.sql.extensions",
            "io.delta.sql.DeltaSparkSessionExtension")
    .config("spark.sql.catalog.spark_catalog",
            "org.apache.spark.sql.delta.catalog.DeltaCatalog")
    # Performance: reduz shuffle overhead (default=200)
    .config("spark.sql.shuffle.partitions", "12")
    .getOrCreate()
)
spark.sparkContext.setLogLevel("WARN")
print(f"   Spark {spark.version} | Delta pronto | shuffle.partitions=12")
...
df = (
    ...
    .persist()   # MEMORY_AND_DISK: evita releitura do MinIO a cada tabela gold
)
...
fat_uf_raw = spark.sql(f"""
    SELECT
        ...
        PERCENTILE_APPROX(vl_liquido, 0.5)         AS vl_mediano,
...
(fat_uf.write.format("delta").mode("overwrite")
    ...
    .save(GOLD_FAT_UF))
print(f"   ✓ fat_uf gravada: {fat_uf.count()} linhas → {GOLD_FAT_UF}")
...
fat_especie = spark.sql(f"""
    ...
    SELECT
        ...
        PERCENTILE_APPROX(b.vl_liquido, 0.5)           AS vl_mediano,
...
print(f"   ✓ fat_especie gravada: {fat_especie.count()} linhas → {GOLD_FAT_ESPECIE}")
...
(fat_banco.write.format("delta").mode("overwrite")
    ...
    .partitionBy("_ano_mes")
    .save(GOLD_FAT_BANCO))
print(f"   ✓ fat_banco gravada: {fat_banco.count()} linhas → {GOLD_FAT_BANCO}")
...
(kpis.write.format("delta").mode("overwrite")
    ...
    .partitionBy("_ano_mes")
    .save(GOLD_KPIS))
print(f"   ✓ kpis_nacionais gravada: {kpis.count()} linha(s) → {GOLD_KPIS}")
...
kpi_row    = spark.read.format("delta").load(GOLD_KPIS).collect()[0]
n_fat_uf   = spark.read.format("delta").load(GOLD_FAT_UF).count()
n_fat_esp  = spark.read.format("delta").load(GOLD_FAT_ESPECIE).count()
n_fat_ban  = spark.read.format("delta").load(GOLD_FAT_BANCO).count()

# Validação cruzada: soma de qtd nas tabelas fat deve bater com kpi_nacionais
sum_qtd_uf    = spark.read.format("delta").load(GOLD_FAT_UF).agg(F.sum("qtd_beneficios")).collect()[0][0]
sum_qtd_esp   = spark.read.format("delta").load(GOLD_FAT_ESPECIE).agg(F.sum("qtd_beneficios")).collect()[0][0]
...
(spark.read.format("delta").load(GOLD_FAT_UF)
    .filter("rank_vl_total <= 5")
    .select("rank_vl_total","sg_uf","qtd_beneficios","vl_total","vl_medio","pct_feminino","pct_nacional_valor")
    .orderBy("rank_vl_total")
    .show(5, truncate=False))

print(f"\n   ── Top 5 Espécies por volume ────────────────────────────")
(spark.read.format("delta").load(GOLD_FAT_ESPECIE)
    .select("especie_codigo","especie_descricao","grupo_especie","qtd_beneficios","vl_total","vl_medio","uf_com_mais_beneficios")
    .orderBy(F.col("qtd_beneficios").desc())
    .show(5, truncate=40))

print(f"\n   ── Market Share Bancário (Top 5) ────────────────────────")
(spark.read.format("delta").load(GOLD_FAT_BANCO)
    .select("rank_qtd","banco_codigo","banco_nome","qtd_beneficios","market_share_pct","market_share_valor_pct")
    .orderBy("rank_qtd")
    .show(5, truncate=False))
